...
gg.manhattan <- function(df_file, threshold = 5e-8, colours = c("black", "grey"), ylims = c(0,100), title = "", highlight_color){
  # read and format df
  df <- df_file
  ...
  df.tmp <- df %>% 
    ...
    mutate( is_highlight=ifelse(SNP %in% hlight & P <= threshold, "yes", "no"))
  ...
  axisdf <- df.tmp %>% group_by(CHR) %>% summarize(center=( max(BPcum) + min(BPcum) ) / 2 )
    ...
    geom_point(data=subset(df.tmp, is_highlight=="yes"), color=highlight_color, size=5) +
