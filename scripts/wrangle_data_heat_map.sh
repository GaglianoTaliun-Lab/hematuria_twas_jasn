...
# create new files to print only tissue, gene name, zscore and pvalues from original outputs:
awk -v tissue="$TISSUE" '{FS = OFS = "\t"} NR==1{print "tissue",$2,$3,$5} NR>1{print tissue, $2, $3, $5}' $HOMEPATH/spredixcan_output/eqtl/hematuria_TOPMed_imputed_eqtl_${TISSUE}.csv > $HOMEPATH/spredixcan_output/heatmap_data/hematuria_TOPMed_imputed_eqtl_full_${TISSUE}.out
awk -v tissue="$TISSUE" '{FS = OFS = "\t"} NR==1{print "tissue",$2,$3,$5} NR>1{print tissue, $2, $3, $5}' $HOMEPATH/spredixcan_output/sqtl/hematuria_TOPMed_imputed_sqtl_${TISSUE}.csv > $HOMEPATH/spredixcan_output/heatmap_data/hematuria_TOPMed_imputed_sqtl_full_${TISSUE}.out
