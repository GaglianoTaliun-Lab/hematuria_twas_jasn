# Description - obtain outcomes for the P3H2 gene across different GWAS summary statistics
...
snps_main <- c("rs11918465","rs12494405","rs9859629","rs838268")
snps_proxy_rs12494405 <- "rs710590"
snps_proxy_rs11918465 <- "rs11915773"
snps_proxy_palindromic <- c("rs9871004","rs6783292")
...
list_all_snps <- c(snps_main, snps_proxy_rs12494405, snps_proxy_palindromic)
...
write.table(hematuria_UKBB, here(project_dir,"MR_analysis","MR_outcomes","hematuria_TOPMed_imputed_P3H2_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(ACR_UKBB, here(project_dir,"MR_analysis","MR_outcomes","ACR_UKBB_P3H2_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(composite_UKBB, here(project_dir,"MR_analysis","MR_outcomes","composite_hematuria_ACR_UKBB_P3H2_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(meta_all_eGFRcrea_stanzick2021, here(project_dir,"MR_analysis","MR_outcomes","metaanalysis_stanzick2021_eGFRcreatinine_P3H2_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(meta_eGFRcrea_stanzick2021, here(project_dir,"MR_analysis","MR_outcomes","metaanalysis_stanzick2021_EUR_eGFRcreatinine_P3H2_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(meta_eGFRcys_stanzick2021, here(project_dir,"MR_analysis","MR_outcomes","metaanalysis_stanzick2021_EUR_eGFRcystatin_P3H2_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)
