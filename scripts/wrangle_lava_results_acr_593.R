...
# read RDS files
bivar <- readRDS(here(project_dir,"MR_analysis","lava","results","593:acr.bivar.lava.rds"))
univar <- readRDS(here(project_dir,"MR_analysis","lava","results","593:acr.univ.lava.rds"))
...
ntest_univ <- read.table(here(project_dir, "MR_analysis","lava", "input_data", "gwas_filtered_593_acr.loci"), sep = "\t", header = T) %>% 
  nrow(.)
...
write.table(bivar_all, here(project_dir,"MR_analysis","lava","results","acr:593_results_bivar.tsv"), sep = "\t", quote = F, row.names = F)
write.table(univ_all, here(project_dir,"MR_analysis","lava","results","acr:593_results_univ.tsv"), sep = "\t", quote = F, row.names = F)

# write significant results into table
write.table(bivar_significant, here(project_dir,"MR_analysis","lava","results","acr:593_significant_results_bivar.tsv"), sep = "\t", quote = F, row.names = F)
write.table(univ_sign, here(project_dir,"MR_analysis","lava","results","acr:593_significant_results_univ.tsv"), sep = "\t", quote = F, row.names = F)
