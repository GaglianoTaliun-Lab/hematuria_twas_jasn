...
# exposure SNPs
snps_original <- c("rs873458","rs61776489")
# not in CKDGen: rs61776489 (proxy: rs2273291)

snps_ckdgen <- c("rs873458","rs2273291")
snps_ukbb <- c("rs873458","rs61776489")
snps_meta <- c("rs873458","rs2273291")
snps_stanzick <- c("rs873458","rs61776489")
...
# 1) Read exposure file for PLOD1 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
exposure_dat <- read_exposure_data(
  filename = here(project_dir,"MR_exposures",'Whole_Blood_EUR_PLOD1_exposure.tsv'),
  sep = '\t',
  snp_col = 'SNP',
  beta_col = 'beta',
  se_col = 'se',
...
outcome_hematuria_UKBB <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","hematuria_TOPMed_imputed_PLOD1_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
outcome_ACR_UKBB <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","ACR_UKBB_PLOD1_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
outcome_composite_UKBB <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","composite_hematuria_ACR_UKBB_PLOD1_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
outcome_meta_stanzick2021_eGFRcrea <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","metaanalysis_stanzick2021_EUR_eGFRcreatinine_PLOD1_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
outcome_meta_stanzick2021_eGFRcys <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","metaanalysis_stanzick2021_EUR_eGFRcystatin_PLOD1_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
mr_results_all_outcomes <- rbind(mr(dat_eGFR_CKDG, method_list = "mr_ivw"),
                                 mr(dat_ACR_CKDG, method_list = "mr_ivw"),
                                 mr(dat_eGFRcys_CKDG, method_list = "mr_ivw"),
                                 mr(dat_hematuria_UKBB, method_list = "mr_ivw"),
                                 mr(dat_ACR_UKBB, method_list = "mr_ivw"),
                                 mr(dat_composite_UKBB, method_list = "mr_ivw"),
                                 mr(dat_meta_ACR_all, method_list = "mr_ivw"),
                                 # mr(dat_meta_ACR_noDM, method_list = "mr_ivw"),
                                 mr(dat_meta_stanzick2021_eGFRcrea, method_list = "mr_ivw"),
                                 mr(dat_meta_stanzick2021_eGFRcys, method_list = "mr_ivw"))
...
mr_heterogeneity(dat_meta_stanzick2021_eGFRcrea)
mr_heterogeneity(dat_meta_stanzick2021_eGFRcys)

# 2) Test for horizontal pleiotropy using MR-Egger intercept test
# I think this test needs > 2 instruments, since I get NA values.
# pleiotropy_test <- mr_pleiotropy_test(dat_meta_stanzick2021_eGFRcrea)
...
LOO <- mr_leaveoneout(dat_meta_stanzick2021_eGFRcrea)
...
pdf(here(project_dir,"figures","PLOD1_scatter_plots.pdf"))
...
write.table(mr_results_all_outcomes, here(project_dir,"results",str_c("MR_results_all_outcomes_PLOD1_",date,".tsv")), sep = "\t", row.names = F, quote = F)
...
mr_report(dat = dat_meta_stanzick2021_eGFRcrea, 
          output_path = here(project_dir,"results"), 
          output_type = "html", 
          author = "FLD", 
          study = "eGFRcrea Meta-analysis from Stanzick et al. 2021 - PLOD1 gene")

mr_report(dat = dat_meta_stanzick2021_eGFRcys, 
          output_path = here(project_dir,"results"), 
          output_type = "html", 
          author = "FLD", 
          study = "eGFRcys Meta-analysis from Stanzick et al. 2021 - PLOD1 gene")
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
ids <- find_cells(mr, 2, c(0:9), "core-fg")
ids2 <- find_cells(mr, 5, c(0:9), "core-fg")
mr <- modify_cells(mr, ids, gpar(fontface="bold"))
mr <- modify_cells(mr, ids2, gpar(fontface="bold"))

# add title:
title <- textGrob("Mendelian Randomization - PLOD1 gene",gp=gpar(fontsize=18))
padding <- unit(5,"mm")
mr <- gtable_add_rows(
  mr, 
...
grid.newpage()
pdf(here(project_dir, "results", str_c("PLOD1_MR_results_table_format_",date,".pdf")), height = 6, width = 15)
grid.draw(mr_table)
dev.off()
