...
gwas_list <-
  setNames(
    object = list(
      ...
      fread(here(project_dir,"MR_analysis","lava", "input_data", "593.lava.gz"))
    ),
    nm = c("acr", "593")
  )
...
write.table(
  loci,
  here(project_dir, "MR_analysis", "lava", "input_data", "gwas_filtered_593_acr.loci"),
  sep = "\t",
  row.names = F,
  quote = F
)
...
saveRDS(
  overlap_df,
  file = here(project_dir, "MR_analysis", "lava", "input_data", "gwas_filtered_593_acr_loci.rds")
)
