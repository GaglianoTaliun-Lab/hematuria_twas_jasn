...
bivar_all <- read.table(here(project_dir,"results","acr:regenie-egfr_results_bivar.tsv"), sep = "\t", header = T)
bivar_bonf <- read.table(here(project_dir,"results","acr:regenie-egfr_significant_results_bivar.tsv"), sep = "\t", header = T)

source(here(project_dir, "plots_RHR.R"))

# get number of bivariate tests
bivar_rds <- readRDS(here(project_dir,"results","acr:regenie-egfr.bivar.lava.rds"))
...
bivar_all %>% 
  ...
  facet_wrap(vars(locus,locus_pos), ncol = 10) +
...
ggsave(here(project_dir, "figures", "acr:regenie-egfr_heatmaps_per_LDblock.pdf"), width = 55, height = 30, units = "cm")
...
pdf(here(project_dir, "figures", "acr:regenie-egfr_LDblock_annotations.pdf"))
fig_list
dev.off()
