# Description - obtain outcomes for the PLOD1 gene across different GWAS summary statistics
...
ACR_UKBB <- fread(here(project_dir,"MR_analysis","ACR_sumstats","ratio.ukb_chr1_v3.SAIGE.txt"))

# Composite phenotype (hematuria + ACR form UKBB):
composite_UKBB <- fread(here(project_dir,"MR_analysis","composite","ratio.casecontrol.ukb_chr1_v3.SAIGE.txt"))
...
list_all_snps <- c(snps_main, snps_proxy_rs61776489)
...
write.table(hematuria_UKBB, here(project_dir,"MR_analysis","MR_outcomes","hematuria_TOPMed_imputed_PLOD1_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(ACR_UKBB, here(project_dir,"MR_analysis","MR_outcomes","ACR_UKBB_PLOD1_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(composite_UKBB, here(project_dir,"MR_analysis","MR_outcomes","composite_hematuria_ACR_UKBB_PLOD1_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(meta_all_eGFRcrea_stanzick2021, here(project_dir,"MR_analysis","MR_outcomes","metaanalysis_stanzick2021_eGFRcreatinine_PLOD1_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(meta_eGFRcrea_stanzick2021, here(project_dir,"MR_analysis","MR_outcomes","metaanalysis_stanzick2021_EUR_eGFRcreatinine_PLOD1_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)

write.table(meta_eGFRcys_stanzick2021, here(project_dir,"MR_analysis","MR_outcomes","metaanalysis_stanzick2021_EUR_eGFRcystatin_PLOD1_outcome.tsv"),
            sep = "\t", row.names = F, quote = F)
