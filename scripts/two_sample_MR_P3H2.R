...
# exposure SNPs
snps_original <- c("rs11918465","rs12494405","rs9859629","rs838268")
# not in CKDGen: rs12494405 (proxy: rs710590)
# palindromic: rs9859629 (proxy: rs9871004)

snps_ckdgen <- c("rs11918465","rs710590","rs9871004","rs838268")
snps_ukbb <- c("rs11918465","rs12494405","rs9871004","rs838268")
snps_meta <- c("rs11915773","rs710590","rs6783292","rs838268")
snps_stanzick <- c("rs11915773","rs710590","rs9871004","rs838268","rs9859629","rs11918465")
...
exposure_dat <- read_exposure_data(
  filename = here(project_dir,"MR_exposures",'NerveTibial_EUR_P3H2_exposure.tsv'),
  sep = '\t',
  snp_col = 'SNP',
  beta_col = 'beta',
  se_col = 'se',
...
# filter exposures to use proxy for the palindromic SNP
...
# a. From the GWAS API for CKDgen data (eGFRcrea): ieu-a-1105 = all inds; ieu-a-1104 = excluding diabetes inds.
...
outcome_hematuria_UKBB <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","hematuria_TOPMed_imputed_P3H2_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
outcome_ACR_UKBB <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","ACR_UKBB_P3H2_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
outcome_composite_UKBB <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","composite_hematuria_ACR_UKBB_P3H2_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
outcome_meta_stanzick2021_eGFRcrea <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","metaanalysis_stanzick2021_eGFRcreatinine_P3H2_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
outcome_meta_stanzick2021_eGFRcys <- read_outcome_data(
  filename = here(project_dir,"MR_outcomes","metaanalysis_stanzick2021_EUR_eGFRcystatin_P3H2_outcome.tsv"),
  phenotype_col = "phenotype",
  chr_col = "chr",
  pos_col = "pos",
  sep = "\t",
...
# g.
dat_meta_ACR_all <- harmonise_data(exposure_dat_meta, outcome_meta_ACR_all, action = 1)
# h.
dat_meta_ACR_noDM <- harmonise_data(exposure_dat_meta, outcome_meta_ACR_noDM, action = 1)
# i.
...
mr_results_all_outcomes <- rbind(mr(dat_eGFR_CKDG, method_list = c("mr_ivw", "mr_egger_regression")),
                                 ...
                                 mr(dat_hematuria_UKBB, method_list = c("mr_ivw", "mr_egger_regression", "mr_weighted_median", "mr_weighted_mode")),
...
# 1) Perform heterogeneity test for eGFR in the CKDGen dataset

# 1a) Heterogeneity test
mr_heterogeneity(dat_hematuria_UKBB)

# 2) Test for horizontal pleiotropy using MR-Egger intercept test for hematuria and ACR in UKBB:
pleiotropy_test <- rbind(mr_pleiotropy_test(dat_hematuria_UKBB), mr_pleiotropy_test(dat_ACR_UKBB))
...
mr_presso(BetaOutcome = "beta.outcome", BetaExposure = "beta.exposure", 
          ...
          OUTLIERtest = TRUE, DISTORTIONtest = TRUE, data = dat_hematuria_UKBB, 
...
pdf(here(project_dir,"figures","P3H2_scatter_plots.pdf"))
...
write.table(mr_results_all_outcomes, here(project_dir,"results",str_c("MR_results_all_outcomes_P3H2_",date,".tsv")), sep = "\t", row.names = F, quote = F)
...
mr_report(dat = dat_hematuria_UKBB, 
          output_path = here(project_dir,"results"), 
          output_type = "html", 
          author = "FLD", 
          study = "Hematuria GWAS (phecode 593) - P3H2 gene")
...
ids <- find_cells(mr, 19, c(0:11), "core-fg")
mr <- modify_cells(mr, ids, gpar(fontface="bold"))

# add title:
title <- textGrob("Mendelian Randomization - P3H2 gene",gp=gpar(fontsize=18))
padding <- unit(5,"mm")
...
grid.newpage()
pdf(here(project_dir, "results", str_c("P3H2_MR_results_table_format_",date,".pdf")), height = 8, width = 14)
grid.draw(mr_table)
dev.off()
