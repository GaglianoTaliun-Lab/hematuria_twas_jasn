...
gmirror_v2 <- function(top, bottom, tline, bline, bline2, chroms = c(1:22, "X", "Y"),log10=TRUE, 
  ...
  if(log10==TRUE){
    ...
    if(!missing(bline2)) {bredline2 <- -log10(bline2)}
  } else {
    ...
    if(!missing(bline2)) {bredline2 <- bline2}
  }
  ...
  if(!missing(bline2)){
    for(i in 1:length(bline2)){
      p2 <- p2 + geom_hline(yintercept = bredline2[i], colour="black", linetype="dashed")
    }
  }
  #Annotate
